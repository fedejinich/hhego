...
type HHESchemeBenchmarkCase struct {
	pastaSecretKey, plaintext                    []uint64
	plainMod, modDegree, secLevel, messageLength uint64
}

func BenchmarkHHEScheme(b *testing.B) {
	for i, bc := range HHESchemeBenchmarksCases() {
		hheBench(b, i, bc.pastaSecretKey, bc.plaintext, bc.plainMod, bc.modDegree, bc.secLevel, bc.messageLength,
			20, 10, true)
	}
}

// benchmark-testing for hhe scheme
func hheBench(b *testing.B, benchIndex int, pastaSecretKey, plaintext []uint64, plainMod, modDegree, secLevel, messageLength,
	...
	// create bfv cipher
	bfv := hhegobfv.NewBFVPastaCipher(modDegree, secLevel, messageLength, bsgN1, bsgN2, useBsGs, plainMod)

	//bfv.printParameters()

	// encrypt plaintext with PASTA
	var pastaCiphertext []uint64
	b.Run(fmt.Sprintf("Bench Plaintext PASTA Encrypt %d. rounds = %d, blocksize = %d, modulus = %d\n",
		benchIndex, pastaParams.Rounds, pastaParams.CiphertextSize, plainMod), func(b *testing.B) {
		for i := 0; i < b.N; i++ {
	...
	b.Run(benchBfvString("Pasta SK BFV Encryption", benchIndex, modDegree, plainMod, uint64(len(pastaSecretKey))), func(b *testing.B) {
		for i := 0; i < b.N; i++ {
	...
	b.Run(benchBfvString("Transcipher", benchIndex, modDegree, plainMod, messageLength), func(b *testing.B) {
		for i := 0; i < b.N; i++ {
	...
	b.Run(benchBfvString("BFV Decrypt", benchIndex, modDegree, plainMod, messageLength), func(b *testing.B) {
		for i := 0; i < b.N; i++ {
	...
	if !util.EqualSlices(decrypted, plaintext) {
		...
		fmt.Printf("pastaSecLevel = %d\n", secLevel)
	}
}

func HHESchemeBenchmarksCases() []HHESchemeBenchmarkCase {
	return []HHESchemeBenchmarkCase{
		{
...
func benchBfvString(name string, benchIndex int, bfvPolyDegree, modulus, messageLength uint64) string {
	return fmt.Sprintf("Bench %s %d, bfv.bfvPolyDegree = %d, bfv.modulus = %d, messageLength = %d\n",
		name, benchIndex, bfvPolyDegree, modulus, messageLength)
}
