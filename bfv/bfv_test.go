...
type BFVTestCase struct {
	secretKey          []uint64
	plaintext          []uint64
	ciphertextExpected []uint64
	bfvPolyDegree      uint64
	modulus            uint64
	pastaSecLevel      uint64
}
...
func TestBfv(t *testing.T) {
	for i, tc := range testCases() {
		...
		t.Run(fmt.Sprintf("Test_Transcipher %d", i), func(t *testing.T) {
			...
			testTranscipher(t, pastaSecretKey, plaintext, ciphertextExpected, modulus, bfvPolyDegree, pastaSecLevel,
				20, 10, true)
		})
	}
...
func testTranscipher(t *testing.T, pastaSecretKey, plaintext, ciphertextExpected []uint64, plainMod, bfvPolyDegree, secLevel,
	...
	messageLength := uint64(len(plaintext))

	if messageLength != uint64(len(ciphertextExpected)) {
	...
	tcs := []BFVTestCase{
		{
			...
			bfvPolyDegree: uint64(math.Pow(2, 14)),
			modulus:       65537,
			pastaSecLevel: 128,
		},
		{
			...
			bfvPolyDegree: uint64(math.Pow(2, 15)),
			modulus:       8088322049,
			pastaSecLevel: 128,
		},
		{
			...
			bfvPolyDegree: uint64(math.Pow(2, 16)),
