...
func NewBFVUtil(bfvParams bfv.Parameters, encoder bfv.Encoder, evaluator bfv.Evaluator) Util {
...
func (bfvUtil *Util) addRcDecomp(state *rlwe.Ciphertext, rc []uint64) {
...
func (bfvUtil *Util) mixDecomp(state *rlwe.Ciphertext) {
...
func (bfvUtil Util) sboxCubeDecomp(state *rlwe.Ciphertext) {
	for i := 0; i < 3; i++ {
		bfvUtil.evaluator.Mul(state, state, state)
	}
}

func (bfvUtil *Util) sboxFeistelDecomp(state *rlwe.Ciphertext, sealParams SealParams) {
	// rotate state
