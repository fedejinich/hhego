...
func TestSingleParty(t *testing.T) {
	for i, tc := range testCases(false) {
		...
		case DecUseCase:
			t.Run(fmt.Sprintf("TestDecUseCase%d", i), func(t *testing.T) {
				//plaintext := hhegobfv.RandomInputV(200, tc.modDegree)
				decTest(t, tc.pastaSecretKey, tc.plaintext, tc.ciphertextExpected, tc.plainMod, tc.modDegree,
					tc.secLevel, tc.bsgN1, tc.bsgN2, tc.useBsGs)
			})
		default:
...
func decTest(t *testing.T, pastaSecretKey, plaintext, ciphertextExpected []uint64, plainMod, modDegree, secLevel,
	...
	//p := pasta.NewPasta(pastaSecretKey, plainMod, PastaParams)
	//e := p.Encrypt(plaintext)
	...
	//b := bfv.Transcipher(e, pastaSKCiphertext)
	//d := bfv.DecryptPacked(&b, uint64(len(plaintext)))
	//if !util.EqualSlices(d, plaintext) {
...
func testCases(enableLargeTestCases bool) []TestCase {
	cases := []TestCase{
		...
		//	//modDegree:  32768, todo(fedejinich) in the original impl it's 2^15, investigate this
		//	modDegree:  65536,
		//	secLevel:   128,
		//	matrixSize: 0,
		//	useBsGs:    true},
		...
		},
		{
			...
			plainMod:   1096486890805657601,
			modDegree:  65536,
			secLevel:   128, // todo(fedejinich) in the original impl it's none
			matrixSize: 200,
			useBsGs:    true,
			bsgN1:      20,
			bsgN2:      10,
