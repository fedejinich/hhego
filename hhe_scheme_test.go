...
func TestFoo(t *testing.T) {
	...
	//rk1 := keygen.GenRelinearizationKeyNew(sk)
	//rk2 := keygen.GenRelinearizationKeyNew(sk)
	//
	//if rk1.Equal(rk2) {
	//	panic("equal")
	//}
	...
	for i := 0; i < 200; i++ {
		op1Ct = ev.MulRelinNew(op1Ct, op1Ct)
		printSizeCt(op1Ct)
	}

	//pastaSecretKey := pastaSK()
	//pastaCipher := pasta.NewPasta(pastaSecretKey, params.T(), PastaParams)
	...
	// original bfv cipher
	//encryptor, decryptor, evaluator, encoder, _, _ := hhegobfv.NewBFVPasta(uint64(params.N()), pasta.DefaultSecLevel,
	//	uint64(len(op1)), 20, 10, params.T(), sk, rlk)

	// server side bfv cipher
	//sk2, _ := keygen.GenKeyPairNew()
	//_, _, ev, enc, _, _ := hhegobfv.NewBFVPasta(uint64(params.N()), pasta.DefaultSecLevel, uint64(len(op1)),
	//	20, 10, params.T(), sk2, rlk)

	//op1Pasta := pastaCipher.Encrypt(op1)
	////op2Pasta := pastaCipher.Encrypt(op2)
	//
	//pastaSKBfv := hhegobfv.EncryptPastaSecretKey(pastaSK(), encoder, encryptor, params)
	//
	//ct := hhegobfv.Transcipher(op1Pasta, pastaSKBfv, PastaParams, pasta.DefaultSecLevel, enc, ev, params)
	//ct2 := hhegobfv.Transcipher(op1Pasta, pastaSKBfv, PastaParams, pasta.DefaultSecLevel, encoder, evaluator, params)
	//
	//dted := hhegobfv.DecryptPacked(&ct, uint64(len(op1)), decryptor, encoder)
	//dted2 := hhegobfv.DecryptPacked(&ct2, uint64(len(op1)), decryptor, encoder)
	//
	//fmt.Println(dted)
	//fmt.Println(dted2)
...
func printSizePt(op *rlwe.Plaintext) {
	opBytes, _ := op.MarshalBinary()
	sizeInBytes := int64(len(opBytes))
	sizeInMB := bytesToMB(sizeInBytes)
	fmt.Printf("%d bytes is %.2f MB\n", sizeInBytes, sizeInMB)
}
func printSizeCt(op *rlwe.Ciphertext) {
	opBytes, _ := op.MarshalBinary()
	sizeInBytes := int64(len(opBytes))
	sizeInMB := bytesToMB(sizeInBytes)
	fmt.Printf("%d bytes is %.2f MB\n", sizeInBytes, sizeInMB)
...
//	secLevel := 128
//	messageLength := 200
//	plaintext := hhegobfv.RandomInputV(messageLength, uint64(65537))
//	bsgN1 := 20
//	bsgN2 := 10
//	useBsGs := true
//
//	hhetest(t, pastaSecretKey, plaintext, uint64(plainMod), uint64(modDegree), uint64(secLevel), uint64(messageLength),
//		uint64(bsgN1), uint64(bsgN2), useBsGs)
//}

func TestHhe2(t *testing.T) {
	pastaSecretKey := []uint64{
		0x07a30, 0x0cfe2, 0x03bbb, 0x06ab7, 0x0de0b, 0x0c36c, 0x01c39, 0x019e0,
		0x0e09c, 0x04441, 0x0c560, 0x00fd4, 0x0c611, 0x0a3fd, 0x0d408, 0x01b17,
		0x0fa02, 0x054ea, 0x0afeb, 0x0193b, 0x0b6fa, 0x09e80, 0x0e253, 0x03f49,
...
func TestHhe3(t *testing.T) {
	...
	secLevel := 128
	messageLength := 200
	plaintext := hhegobfv.RandomInputV(messageLength, 8088322049)
	useBsGs := true
	bsgN1 := 20
	bsgN2 := 10
...
func hhetest(t *testing.T, pastaSecretKey, message []uint64, plainMod, polyDegree, secLevel, messageLength,
	...
	// create bfv cipher
	encryptor, decryptor, evaluator, encoder, _, _ := hhegobfv.NewBFVPasta(polyDegree, secLevel, messageLength, bsgN1, bsgN2, plainMod, sk, rk)
